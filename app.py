...
def generate_cards(sensor='Temperature'):
    cards = html.Div(
        [
            dbc.Row(
                [
                    dbc.Col(dbc.Card(generate_card_content("Temperature",get_sensor_value(data_sensors_ts,'Temperature'),get_sensor_total('Temperature')), color = color_condition('Temperature',get_sensor_value(data_sensors_ts,'Temperature')), inverse=True),md=dict(size=2,offset=2)),
